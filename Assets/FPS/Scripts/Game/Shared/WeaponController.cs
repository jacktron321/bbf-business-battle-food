...
namespace Unity.FPS.Game
{
    ...
    {
        public UnityAction noAmmo;
        bool invokeSent = false;
        public bool startReload = false;
        bool canShoot = true;
        [Header("Weapon data")]
        //[Tooltip("Amount of health when consume")]
        //public int healAmount = 0;
        [Tooltip("Amount of damage dealt")]
        public float dmgAmount = 0;
        public float DmgMult = 1;
        public float ATSMult = 1;
        [Tooltip("Price of the weapon in shop")]
        public int weaponPrice = 20;
        [Tooltip("Time to consume the weapon in seconds")]
        ...
        [Tooltip("The alternative projectile prefabs")] public ProjectileBase[] AltProjectilePrefab;
        ...
        [Tooltip("sound played when eating")]
        ...
        void Awake()
        {
            ...
            if(WeaponMuzzle) m_LastMuzzlePosition = WeaponMuzzle.position;
            ...
            if (weaponType == WeaponType.Melee){
                ...
                //DebugUtility.HandleErrorIfNullGetComponent<ColliderDetector, WeaponController>(colliderDetector, this,
                //gameObject);
                DeactivateColliderWeapon();
            }
        ...
        void Update()
        {
            ...
            if(m_CurrentAmmo == 0 && !invokeSent) {
                noAmmo?.Invoke();
                invokeSent = true;
            }else if(m_CurrentAmmo != 0) invokeSent = false;
        }
        ...
        void UpdateAmmo()
        {
            ...
            else if (!AutomaticReload && m_LastTimeShot + AmmoReloadDelay < Time.time && m_CurrentAmmo < MaxAmmo && !IsCharging && startReload)
            {
                // reloads weapon over time
                m_CurrentAmmo += AmmoReloadRate * Time.deltaTime;
        ...
        {
            if (m_CurrentAmmo >= 1f
                && m_LastTimeShot + (DelayBetweenShots/ATSMult) < Time.time)
            {
                HandleShoot();
                m_CurrentAmmo -= 1f;

                return true;
            } else if (!(m_CurrentAmmo >= 1f)){
                if(Flame){
        ...
        void HandleShoot()
        {
            if (weaponType == WeaponType.Range){
                ...
                if(AltMuzzle != null){
                    int ind = 0;
                    foreach(Transform alt_muzzle in AltMuzzle){
                        Vector3 shotDirection = GetShotDirectionWithinSpread(alt_muzzle);
                        ProjectileBase newProjectile = Instantiate(AltProjectilePrefab[ind], alt_muzzle.position,
                            Quaternion.LookRotation(shotDirection));
                        newProjectile.Shoot(this);
                        ind+=1;
                    }
                ...
                if (WeaponAnimator)
                {
                    if(WeaponName == "Pineapple"){
                        if(!Punch){
                            Punch = !Punch;
                            WeaponAnimator.SetTrigger(k_AnimAttackParameter);
                        }else {
                            Punch = !Punch;
                            WeaponAnimator.SetTrigger(k_AnimAttackParameter+"2");
                        }

                    }else WeaponAnimator.SetTrigger(k_AnimAttackParameter);
                    
                }
                //isAttacking = false;
                OnShoot?.Invoke();
                OnShootProcessed?.Invoke();
                //colliderDetector.DetectHit(this.Owner);
                //isAttacking = false;
        ...
        /// De momento está aqui
        /*public int HandleConsume()
        {
            if (m_CurrentAmmo >= 1f)
            {
                int heal_ammount = healAmount;
                // Trigger consume animation if there is any
                if (WeaponAnimator)
                {
                    WeaponAnimator.SetTrigger(k_AnimAttackParameter); // Cambiar
                }
        ...
        public void HandleEatingSound(bool consuming){
            if(consuming){
                if(!m_ShootAudioSource.isPlaying){
                    if(EatingSfx) m_ShootAudioSource.PlayOneShot(EatingSfx);
                } 
            } else m_ShootAudioSource.Stop();
        }
        ...
        public void DeactivateColliderWeapon(){
            //Debug.Log("Desactivar");
            Collider col = this.GetComponent<Collider>();
            col.enabled = false;
        }
        public void AtivateColliderWeapon(){
            //Debug.Log("Activar");
            //Debug.Log(this.name);
            //Debug.Log(this.m_CurrentAmmo);
            Collider col = this.GetComponent<Collider>();
            //Debug.Log(col.name);
            //Collider col = FindObjectOfType<Collider>();
            //Debug.Log(col.name);
