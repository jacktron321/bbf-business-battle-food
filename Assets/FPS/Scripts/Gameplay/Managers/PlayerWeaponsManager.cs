...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        //[Tooltip("Time to consume the weapon")]
        //public float timeToConsume = 3.0f;
        [Tooltip("Ammount of weapon slots")]
        ...
        WeaponController[] m_WeaponSlots = new WeaponController[maxSlots]; // n available weapon slots
        ...
        public float baseTime = 0.7f;
        public bool isConsuming = false;
        Shop shop;

        public BuffManager buff_manag;

        void Start()
        {
            shop = FindObjectOfType<Shop>();
            DebugUtility.HandleErrorIfNullGetComponent<Shop, PlayerWeaponsManager>(shop, this,
                gameObject);
            ...
            buff_manag = GetComponent<BuffManager>();
            DebugUtility.HandleErrorIfNullGetComponent<BuffManager, PlayerWeaponsManager>(buff_manag, this,
                gameObject);
        ...
        void Update()
        {
            ...
            {
                activeWeapon.DmgMult = buff_manag.DmgMult;
                activeWeapon.ATSMult = buff_manag.ATSMult;
                ...
                */
                if(!isConsuming){
                    ...
                    if(activeWeapon.WeaponName == "Chili"){
                        activeWeapon.Flame.Stop();
                    }
                }
            ...
            // weapon consume handling
            if (activeWeapon != null)
            {
                if (m_InputHandler.GetConsumeInputHeld() || isConsuming)
                {
                    
                    // Check if player CAN consume weapon
                    //Health player_h = m_PlayerCharacterController.GetComponent<Health>();
                    //if (player_h.CurrentHealth < player_h.MaxHealth){
                    consumeTimer += Time.deltaTime;
                    isConsuming = true;
                    activeWeapon.HandleEatingSound(isConsuming);
                    //if (consumeTimer >= timeToConsume){
                    if (consumeTimer >= Mathf.Max((activeWeapon.timeToConsume*activeWeapon.CurrentAmmoRatio),baseTime)){
                        consumeTimer = 0;
                        //IsAiming = false;
                        isConsuming = false;
                        //buff_manag.ActivateBuff(activeWeapon,m_PlayerCharacterController);
                        if(activeWeapon.Buff) buff_manag.ActivateBuff(activeWeapon);
                        if(activeWeapon.Debuff)buff_manag.ActivateDebuff(activeWeapon);
                        RemoveWeapon(activeWeapon);
                        /*int hasConsumed = activeWeapon.HandleConsume();
                        if(hasConsumed >= 0){
                            player_h.Heal(hasConsumed);
                            RemoveWeapon(activeWeapon);
                        }*/
                    }

                    if(m_InputHandler.GetConsumeInputReleased()){
            ...
            if ((activeWeapon == null || !activeWeapon.IsCharging) &&
                (m_WeaponSwitchState == WeaponSwitchState.Up || m_WeaponSwitchState == WeaponSwitchState.Down) && !shop.isOpen)
            {
                int switchWeaponInput = m_InputHandler.GetSwitchWeaponInput();
                if ((switchWeaponInput != 0) && !isConsuming)
                {
        ...
        public bool CheckWeaponList(){
            ...
            foreach (WeaponController wp in m_WeaponSlots){
                if (wp!=null) ammount +=1;
            }
            if (ammount == 2) return false;
            return true;
        }
            ...
            if (m_WeaponSwitchState == WeaponSwitchState.Up)
            {
                ...
                if(activeWeapon){
                    m_WeaponMainLocalPosition = Vector3.Lerp(m_WeaponMainLocalPosition,
                        DefaultWeaponPosition.localPosition, 10f * Time.deltaTime);
        ...
        public bool AddWeapon(WeaponController weaponPrefab)
        {
            ...
            for (int i = 0; i < m_WeaponSlots.Length; i++)
            {
                ...
                if (m_WeaponSlots[i] == null)
                {
                    ...
                    if(weaponPrefab.tag == "Pineapple") weaponInstance.tag = "Pineapple";
                    ...
                    if(i == 0){
                        //Debug.Log("Aqui 1");
                        if(m_WeaponSlots[i+1] == null){
                            //Debug.Log("Aqui 2");
                            weaponInstance.ShowWeapon(true);
                        }else weaponInstance.ShowWeapon(false);
                    }
                    else {
