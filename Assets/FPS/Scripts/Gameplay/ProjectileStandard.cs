...
namespace Unity.FPS.Gameplay
{
    public class ProjectileStandard : ProjectileBase
    {
        ...
        [Tooltip("Auxiliar damage, the real damage is in the weapon")]
        public float Damage = 1;
        public float DamageMult = 1;

        [Tooltip("Area of damage. Keep empty if you don<t want area damage")]
        public DamageArea AreaOfDamage;
        [Tooltip("Does Proyectile destroy on hit")]
        ...
        new void OnShoot()
        {
            ...
            Damage = m_ProjectileBase.Dmg;
            DamageMult = m_ProjectileBase.DmgMult;
        ...
        void Update()
        {
            ...
            // Hit detection
            {
                ...
                if (foundHit)
                {
                    ...
                    //LastCollider = closestHit.collider;
                    //Debug.Log(this.name);
        ...
        void OnHit(Vector3 point, Vector3 normal, Collider collider)
        {
            ...
            else
            {
                ...
                //{
                    //Debug.Log(this.name);
                    //if(this.name == "Projectile_Pepperoni(Clone)" && this.name == "Projectile_Donut(Clone)"){
                    //Debug.Log(DestroyonHit);
                    if(DestroyonHit == true){
                        //Debug.Log("Does Destroy");
                        Damageable damageable = collider.GetComponent<Damageable>();
                        if(damageable) damageable.InflictDamage(Damage*DamageMult, false, m_ProjectileBase.Owner);
                    } else if (DestroyonHit == false) {
                        //Debug.Log("Doesn't Destory");                                      
                        //if(LastCollider && penDamage){
                        if(penDamage){
                            //Debug.Log("Aqui 2");
                            /*
                            Debug.Log(LastCollider.name);
                            Debug.Log(collider.name);
                            Debug.Log(LastCollider != collider);
                            if(LastCollider != collider){*/
                            Damageable damageable = collider.GetComponent<Damageable>();
                            if(damageable){
                                //Debug.Log("Pen Dmg");
                                damageable.InflictDamage(Damage*DamageMult, false, m_ProjectileBase.Owner);
                                penDamage = false;
                                StartCoroutine(waitpenDmg(tickRate));
                            }
                        }
            ...
            if(this){
                if((collider.tag == "Solid") && DestroyOnGround) {
                    if (ImpactVfx && DestroyOnGround)
                    {
                        GameObject impactVfxInstance = Instantiate(ImpactVfx, point + (normal * ImpactVfxSpawnOffset),
                            Quaternion.LookRotation(normal));
        ...
        IEnumerator waitpenDmg(float time){
